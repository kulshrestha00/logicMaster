...
<body class="bg-gray-100 font-family-karla flex body">
    ...
    <aside class="relative bg-sidebar h-10 w-65 hidden sm:block shadow-xl" style=" height: 625px; overflow-y:scroll;"> 
        ...
        <nav class="text-white text-base font-semibold pt-3">
            ...
            <a href="jdbc.jsp" class="flex items-center active-nav-link text-white py-4 pl-6 nav-item">
              JDBC
            </a> 
 ...
 -->
    ...
    <div class="w-full overflow-x-hidden border-t flex flex-col">
        <main class="w-full flex-grow p-6 main">
            <!-- Writing area -->
     <h1 class="text-3xl text-black pb-6" style="font-size: 50px ;font-family:sans-serif; color: #50006b" >Introduction to JDBC</h1>  <!-- This is the Heading section.. dont do any changes except changing the heading-->
            <hr style="color: black">
            <br>
               <iframe width="560" height="315" src="https://www.youtube.com/embed/y_YxwyYRJek" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            <p style="font-size: 18px"> 
             JDBC is an acronym for Java Database Connectivity. It’s an advancement for ODBC ( Open Database Connectivity ). JDBC is an standard API specification developed in order to move data from frontend to backend.<br>This API consists of classes and interfaces written in Java. It basically acts as an interface (not the one we use in Java) or channel between your Java program and databases i.e it establishes a link between the two so that a programmer could send data from Java code and store it in the database for future use.
                 <br>
<br><b style="font-size: 18px">Why JDBC came into existence ?</b><br>
...
<br>
             <br><h1 style="font-size: 25px">Steps for connectivity between Java program and database</h1>
<br>

  <ul style="list-style-type:decimal; padding-left:1em">
  <li style="font-size: 18px">
    <b> Loading the Driver</b><br>
      To begin with, you first need load the driver or register it before using it in the program . Registration is to be done once in your program. You can register a driver in one of two ways mentioned below :
          <br><br><ul style="list-style-type:lower-alpha; padding-left:5em">
            <li style="font-size: 18px">
              <b>Class.forName()</b><br>Here we load the driver’s class file into memory at the runtime. No need of using new or creation of object .The following example uses Class.forName() to load the Oracle driver –
              <div style="background-color: #D3D3D3; padding-top: 5px; padding-bottom: 5px; width: 500px; padding-left: 25px">Class.forName(“oracle.jdbc.driver.OracleDriver”);</div>
            </li>
            <li style="font-size: 18px">
               <b>DriverManager.registerDriver()</b><br>DriverManager is a Java inbuilt class with a static member register. Here we call the constructor of the driver class at compile time . The following example uses DriverManager.registerDriver()to register the Oracle driver –
               <div style="background-color: #D3D3D3; padding-top: 5px; padding-bottom: 5px; width: 600px; padding-left: 25px">DriverManager.registerDriver(new oracle.jdbc.driver.OracleDriver());</div>
            </li>
          </ul>
      </li><br><br>
      <li style="font-size: 18px"> 
        <b> Create the connections</b><br>After loading the driver, establish connections using :<br>
        <div style="background-color: #D3D3D3; padding-top: 5px; padding-bottom: 5px; width: 600px; padding-left: 25px">Connection con = DriverManager.getConnection(url,user,password)</div>
        <br>
        <b>user</b> – username from which your sql command prompt can be accessed.
        <br><b>password</b> – password from which your sql command prompt can be accessed.
        <br><b>con</b> - is a reference to Connection interface.
        <br><b>url </b> - Uniform Resource Locator. It can be created as follows:
        <div style="background-color: #D3D3D3; padding-top: 5px; padding-bottom: 5px; width: 600px; padding-left: 25px">String url = “ jdbc:oracle:thin:@localhost:1521:xe”</div>
        <br>Where oracle is the database used, thin is the driver used , @localhost is the IP Address where database is stored, 1521 is the port number and xe is the service provider. All 3 parameters above are of String type and are to be declared by programmer before calling the function. Use of this can be referred from final code.
      </li><br><br>
      <li style="font-size: 18px"> 
        <b>Create a statement</b><br>Once a connection is established you can interact with the database. The JDBCStatement, CallableStatement, and PreparedStatement interfaces define the methods that enable you to send SQL commands and receive data from your database.<br>
Use of JDBC Statement is as follows:
        <div style="background-color: #D3D3D3; padding-top: 5px; padding-bottom: 5px; width: 600px; padding-left: 25px">Statement st = con.createStatement();</div>
      </li><br><br>
      <li style="font-size: 18px"> 
        <b>Execute the query</b><br>Now comes the most important part i.e executing the query. Query here is an SQL Query . Now we know we can have multiple types of queries. Some of them are as follows:
        <ul style="list-style-type:circle; padding-left:5em"><li>Query for updating / inserting table in a database.</li><li>Query for retrieving data </li></ul>
        <br>The executeQuery() method of Statement interface is used to execute queries of retrieving values from the database. This method returns the object of ResultSet that can be used to get all the records of a table.<br>
The executeUpdate(sql query) method ofStatement interface is used to execute queries of updating/inserting.
<br><b>Example :</b><br>
<div style="background-color: #D3D3D3; padding-top: 5px; padding-bottom: 5px; width: 600px; padding-left: 25px">
  int m = st.executeUpdate(sql);<br>
if (m==1)<br>
    System.out.println("inserted successfully : "+sql);<br>
else<br>
    System.out.println("insertion failed");<br>
</div>
      </li><br><br>
      <li style="font-size: 18px"> 
        <b>Close the connections</b><br>So finally we have sent the data to the specified location and now we are at the verge of completion of our task .<br>
...
<b>Example :</b><br>
<div style="background-color: #D3D3D3; padding-top: 5px; padding-bottom: 5px; width: 600px; padding-left: 25px">con.close();  </div>
      </li>
    </ul><br><br>

    <mark><b>This is just the overview of jdbc. We will soon introduce a new course of jdbc.</b></mark>
<br><br>
         <hr>
                     <!-- This should not be in the front page but for example i am writing this here for reference  -->
           <a href="interfaces.jsp"> <button class="ml-4 inline-flex text-gray-400 bg-gray-800 border-0 py-2 px-6 focus:outline-none hover:bg-gray-700 hover:text-white rounded text-lg" >Previous page</button></a>
