...
<body class="bg-gray-100 font-family-karla flex body">
    ...
    <aside class="relative bg-sidebar h-10 w-65 hidden sm:block shadow-xl" style=" height: 625px; overflow-y:scroll;"> 
        ...
        <nav class="text-white text-base font-semibold pt-3">
            ...
            <a href="readerAndWriter.jsp" class="flex items-center active-nav-link text-white py-4 pl-6 nav-item">
 ...
 -->
    ...
    <div class="w-full overflow-x-hidden border-t flex flex-col">
        <main class="w-full flex-grow p-6 main">
            <!-- Writing area -->
            <h1 class="text-3xl text-black pb-6" style="font-size: 50px ;font-family:sans-serif; color: #50006b" >Reader and Writer</h1>  <!-- This is the Heading section.. dont do any changes except changing the heading-->

            <iframe width="560" height="315" src="https://www.youtube.com/embed/SslMi6ptwH8" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            <hr style="color: black">
            <br>

            <p style="font-size: 18px">
              The Java Reader (java.io.Reader) and Java Writer class (java.io.Writer) in Java IO work much like the InputStream and OutputStream with the exception that Reader and Writer are character based. They are intended for reading and writing text. The InputStream and OutputStream are byte based, remember?</p>
              <br>
              <h1 style="font-size: 35px">Reader</h1>
              <p style="font-size: 18px">The Java Reader is the base class of all Reader's in the Java IO API. Subclasses include a BufferedReader, PushbackReader, InputStreamReader, StringReader and several others.
            <br>
            Here is a simple Java IO Reader example:
              </p>
              <br>
              <div style="border-style: groove; height: 210px; width: 300px; background-color: #D3D3D3; padding-left: 20px; padding-top: 10px; align-content: center;" >
                    Reader reader = new FileReader("c:\\data\\myfile.txt");<br>
<br>
    int data = reader.read();<br>
    while(data != -1){<br>
        char dataChar = (char) data;<br>
        data = reader.read();<br>
    }<br>
              </div>
              <br>
                <p style="font-size: 18px">Notice, that while an InputStream returns one byte at a time, meaning a value between 0 and 255 (or -1 if the stream has no more data), the Reader returns a char at a time, meaning a value between 0 and 65535 (or -1 if the stream has no more data). This does not necessarily mean that the Reader reads two bytes at a time from the source it is connected to. It may read one or more bytes at a time, depending on the encoding of the text being read.</p>
                    <br><br>
                <h1 style="font-size: 35px">Combining Readers With InputStreams</h1>
                <br>
                <p style="font-size: 18px">A Java Reader can be combined with an InputStream. If you have an InputStream and want to read characters from it, you can wrap it in an InputStreamReader. Pass the InputStream to the constructor of the InputStreamReader like this:</p><br>
                  <div style="border-style: groove; height: 40px; width: 500px; background-color: #D3D3D3; padding-left: 20px; padding-top: 10px; align-content: center;" >Reader reader = new InputStreamReader(inputStream);</div><br>
                  <p style="font-size: 18px">In the constructor you can also specify what character set to use to decode the text etc. More on that in the text on InputStreamReader.</p>
                    <br><br>
                  <h1 style="font-size: 35px">Writer</h1>
                  <p style="font-size: 18px">The Java Writer class is the base class of all Writers in the Java IO API. Subclasses include BufferedWriter and PrintWriter among others.
                    <br>
Here is a simple Java IO Writer example:</p>
                    <div style="border-style: groove; height: 130px; width: 500px; background-color: #D3D3D3; padding-left: 20px; padding-top: 10px; align-content: center;" >Writer writer = new FileWriter("c:\\data\\file-output.txt");<br>
<br>
writer.write("Hello World Writer");<br>
writer.close();<br></div>
<br>
<br>
                 <h1 style="font-size: 35px"> Combining Writers With OutputStreams</h1>
                 <br>
                 <p style="font-size: 18px">A Java Writer can be combined with an OutputStream just like Readers and InputStream's. Wrap the OutputStream in an OutputStreamWriter and all characters written to the Writer are passed on to the OutputStream. Here is an OutputStreamWriter example:</p>
                 <br><div style="border-style: groove; height: 40px; width: 500px; background-color: #D3D3D3; padding-left: 20px; padding-top: 10px; align-content: center;" >Writer writer = new OutputStreamWriter(outputStream);</div>
                 <br>
                 <br>
                 <h1 style="font-size: 35px"> Combining Readers and Writers</h1>
                 <br>
                 <p style="font-size: 18px"> Just like with streams, Reader's and Writer's can be combined into chains to achieve more interesting IO. It works just like combining the Reader with InputStream's or the Writer with OutputStream's. For instance, you can achieve buffering by wrapping a Reader in a BufferedReader, or a Writer in a BufferedWriter. Here are two such examples:</p><br>
                  <div style="border-style: groove; height: 80px; width: 500px; background-color: #D3D3D3; padding-left: 20px; padding-top: 10px; align-content: center;" >
                    Reader reader = new BufferedReader(new FileReader(...));<br>

Writer writer = new BufferedWriter(new FileWriter(...))</div>
              
<br><br>
         <hr>
                     <!-- This should not be in the front page but for example i am writing this here for reference  -->
           <a href="file.jsp"> <button class="ml-4 inline-flex text-gray-400 bg-gray-800 border-0 py-2 px-6 focus:outline-none hover:bg-gray-700 hover:text-white rounded text-lg" >Previous page</button></a>
           <a href="buffered.jsp"><button class="inline-flex text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg" style="float: right;">Next page</button></a>
